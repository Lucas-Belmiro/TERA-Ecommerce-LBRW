...
const InfoCompraFinal = () => {
    ...
    function atualizaUsuario(dado) {
            ...
            if (res.status == 200) {
                res.json().then((data2) => {
                    ...
                    localStorage.setItem(
                        'idCompras',
                        JSON.stringify(comprasPassadas)
                    )
                })
            } else {
                console.log('Não foi enviado :(')
        ...
        const compraAnterior = JSON.parse(
            localStorage.getItem('idCompras') || []
        )
    ...
    function EnviarDados(evento) {
        ...
        if (botao1 === true && botao2 === true) {
            ...
            fetch('https://lucasbelmiro.com/dados/salvar', options).then(
                (res) => {
                    if (res.status == 200) {
                        res.json().then((data) => {
                        ...
                        <div>
                            ...
                            <p>
                                ...
                                {produtosExistentes.map(
                                    (produto) =>
                                        `(${produto.price}, ${produto.name}) `
                                )}{' '}
                            </p>
                            <p>Frete: R$ 25,00</p>
                            <hr className="separador"></hr>
                            <p className="total-carrinho">
                                Total: R$ {somaDeValores},00
                            </p>
                            <Button
